...
一个运行在本机的中文 Web Agent。你在网页中直接对话，或选定工作区后提出任务，Pi SDK 负责模型会话、工具调用和插件执行，生成的文件保存在会话专属目录或所选工作区，并可在网页下载。多步骤任务会展示并保存执行计划，意外中断后可以继续；工具或模型调用失败时会先分析原因，只对瞬时故障执行有上限的退避重试。项目还提供可选的本地文档知识库，适合一边处理项目文件，一边查询自己的资料。
...
1. 打开“对话”即可发送消息，无需选择文件夹。需要操作已有项目时，在顶部输入文件夹的绝对路径并点击“应用”，或点击“选择文件夹”；点击“网页对话”可切回无文件夹模式。
...
网页对话生成的文件保存在 `tmp/chat-workspaces/<用户>/<会话>/`，不同会话使用独立目录，历史会话的成果仍可下载。成果列表只包含 `deliver_files` 明确登记的最终文件，不会自动附带所有写入文件或中间脚本。
...
│  │  └─ collect-artifacts.ts  # 已登记最终成果的交付校验
